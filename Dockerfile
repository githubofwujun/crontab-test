...
RUN yum install -y wget which telnet bind-utils net-tools cronie cronie-anacron gzip MariaDB-client socat  && \
    ...
    echo '*/1 * * * * root  echo -e "$(date +'%F %T')\tcrond works" >> /tmp/cron.txt' >> /etc/crontab && \
    ...
    yum clean all && \
    [ -f "/opt/scripts/backup-restore.sh" ] && echo '*/10 * * * * root  sh /opt/scripts/backup-restore.sh' >> /etc/crontab
