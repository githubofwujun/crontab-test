...
RUN yum install -y wget which telnet bind-utils net-tools cronie crontabs gzip MariaDB-client socat  && \
    ...
    yum clean all && \
    [ -d "/var/spool/cron" ] && echo '*/1 * * * * root   echo $(pwd) >> /tmp/pwd.txt' >> /var/spool/cron/root 
